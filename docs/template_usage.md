...
:::{note}
If you are looking at this document in the [cookiecutter-scverse-instance][] repository documentation, throughout this document the name of the project is `cookiecutter-scverse-instance`. Otherwise it should be replaced by your new project name: `reproducibility-template`.
:::

Now that your new project repository has been created on github at `https://github.com/martinkim0/reproducibility-template` you can push your first commit to github.
To do this, simply follow the instructions on your github repository page or a more verbose walkthrough here:

Assuming you are in `/your/path/to/reproducibility-template`. Add all files and commit.
...
git remote add origin https://github.com/martinkim0/reproducibility-template.git
...
Your project should be now available at `https://github.com/martinkim0/reproducibility-template`. While the repository at this point can be directly used, there are few remaining steps that needs to be done in order to achieve full functionality.
...
The workflows should now succeed and you will be able to find the code coverage at this link: `https://app.codecov.io/gh/martinkim0/reproducibility-template`. You might have to wait couple of minutes and the coverage of this repository should be ~60%.
...
+
 [tool.coverage.run]
 source = ["reproducibility-template"]
 omit = [
```
